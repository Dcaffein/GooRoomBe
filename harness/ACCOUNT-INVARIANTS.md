...
프론트가 409에서 "다시 시도"를 유도하면 안 된다 — 대응은 "로그인"이다.

**409의 원인은 둘이고 응답으로 구분되지 않는다.**

| 원인 | 예시 |
|---|---|
| 다른 경로(OAuth)로 가입됨 | 링크를 여는 사이 같은 이메일로 구글 가입 |
| **같은 이메일의 다른 로컬 토큰으로 먼저 완료됨** | 두 번 접수해 토큰 A로 가입을 마친 뒤 B를 제출 (I-5) |

둘 다 `AlreadyRegisteredEmailException` 하나로 나가므로 **"구글로 로그인하세요"라고 안내하면 안 된다.**
두 번째 경우 구글은 개입한 적이 없고, 그 사용자는 구글 계정이 없을 수 있다.
문구는 **"이미 가입이 완료된 이메일입니다. 로그인해 주세요"** 처럼 경로를 특정하지 않아야 한다.
